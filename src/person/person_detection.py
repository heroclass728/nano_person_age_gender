...
def detect_person():
    ...
    net = cv2.dnn.readNetFromCaffe(CAFFE_PROTEXT, CAFFE_MODEL)
    ...
    while True:
        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence > DETECTION_CONFIDENT:
        ...
        for (objectID, centroid) in objects.items():
            # check to see if a traceable object exists for the current object ID
            to = traceable_objects.get(objectID, None)

            # if there is no existing traceable object, create one
            if to is None:
                to = TraceableObject(objectID, centroid)
            ...
            # to determine direction
                ...
                if not to.counted:
                    # if the direction is negative (indicating the object is moving up) AND the centroid is
                    # above the center line, count the object
                    total += 1
                    to.counted = True

            # store the traceable object in our dictionary
            traceable_objects[objectID] = to
