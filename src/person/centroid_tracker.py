...
class CentroidTracker:
	...
	def __init__(self, max_disappeared=50, max_distance=50):
		...
		# initialize the next unique object ID along with two ordered dictionaries used to keep track of
		# mapping a given object ID to its centroid and number of consecutive frames it has been marked
		# as "disappeared", respectively
	...
	def update(self, rects):
		...
		# existing object centroids
			...
			dist_ins = dist.cdist(np.array(objectCentroids), inputCentroids)

			# in order to perform this matching we must (1) find the smallest value in each row and
			# then (2) sort the row indexes based on their minimum values so that the row
			# with the smallest value as at the *front* of the index list
			rows = dist_ins.min(axis=1, initial=None).argsort()

			# next, we perform a similar process on the columns by finding the smallest value in each column and then
			# sorting using the previously computed row index list
			cols = dist_ins.argmin(axis=1)[rows]
			...
			for (row, col) in zip(rows, cols):
				...
				if dist_ins[row, col] > self.maxDistance:
					continue

				# otherwise, grab the object ID for the current row, set its new centroid, and reset the disappeared
				# counter
				object_id = objectIDs[row]
				self.objects[object_id] = inputCentroids[col]
			...
			unusedRows = set(range(0, dist_ins.shape[0])).difference(usedRows)
			unusedCols = set(range(0, dist_ins.shape[1])).difference(usedCols)
			...
			if dist_ins.shape[0] >= dist_ins.shape[1]:
				...
				for row in unusedRows:
					...
					# for warrants deregister the object
					if self.disappeared[object_id] > self.maxDisappeared:
						self.deregister(object_id)

			# otherwise, if the number of input centroids is greater than the number of existing object centroids we need to
			# register each new input centroid as a traceable object
			else:
				for col in unusedCols:
					self.register(inputCentroids[col])

		# return the set of traceable objects
		return self.objects
